...
class SmartSearchService {
  ...
  static const Map<String, List<String>> _jobSynonyms = {
    ...
    'سائق': ['سواق', 'توصيل', 'نقل عفش', 'ترحيل', 'مشاوير', 'امجاد', 'أمجاد', 'ركشة', 'طرحة', 'هايس', 'دفار'],
    'نقل': ['سائق', 'سواق', 'توصيل', 'ناقل', 'ترحيل', 'نقل عفش', 'دفار', 'ركشة'],
    
    // الطبخ والأكل
    'طباخ': ['طبخ', 'شيف', 'طاهي', 'صناعة طعام', 'طباخة', 'اكل بيتي', 'عصيدة', 'كسرة'],
    'شيف': ['طباخ', 'طبخ', 'طاهي'],
    ...
    // الأجهزة الإلكترونية
    'إلكترونيات': ['اجهزه', 'أجهزة', 'لابتوب', 'حاسوب', 'كمبيوتر', 'شاشات', 'صيانة اجهزة', 'رسيفر', 'دش'],
    'كمبيوتر': ['لابتوب', 'حاسوب', 'إلكترونيات', 'صيانة كمبيوتر', 'لبتوب'],
    
    // صيانة الهواتف المدمجة
    'صيانة موبايل': ['صيانة هواتف', 'تصليح موبايلات', 'صيانة جوالات', 'صيانة تلفونات', 'تصليح هواتف', 'صيانة موبايلات', 'سوفت وير'],
    'صيانة هواتف': ['صيانة موبايل', 'تصليح موبايلات', 'صيانة جوالات', 'صيانة تلفونات', 'تصليح هواتف', 'صيانة موبايلات', 'بتاع تلفونات'],
  ...
  static bool _isCloseMatch(String word, String query) {
    ...
    if (cleanWord.isNotEmpty && cleanQuery.isNotEmpty && cleanQuery.length >= 3) {
      if (cleanWord.contains(cleanQuery) && cleanWord.length - cleanQuery.length <= 2) return true;
      if (cleanQuery.contains(cleanWord) && cleanQuery.length - cleanWord.length <= 2) return true;
    }

...
class FirestoreService {
  ...
  Future<void> addStoryViewer(String storyId, String userId) => _story.addViewer(storyId, userId);
  Future<void> deleteStory(String storyId) => _story.deleteStory(storyId);

  // ==================== PORTFOLIO ====================
  Future<String> addPortfolioProject(PortfolioProjectModel project) => _portfolio.addProject(project);
  Stream<List<PortfolioProjectModel>> getUserPortfolio(String userId) => _portfolio.getUserProjects(userId);
  Future<void> deletePortfolioProject(String userId, String projectId) => _portfolio.deleteProject(userId, projectId);
  Future<void> updatePortfolioProject(String userId, String projectId, Map<String, dynamic> data) => _portfolio.updateProject(userId, projectId, data);

  // ==================== USERS ====================
  Future<UserModel?> getUser(String userId) => _users.getUser(userId);
  Stream<UserModel?> getUserStream(String userId) => _users.getUserStream(userId);
  Future<void> updateUserProfile(String userId, Map<String, dynamic> data) => _users.updateUserProfile(userId, data);
  ...
  // ==================== REVIEWS ====================
  Future<String> createReview(ReviewModel review, {bool isJobCompleted = false}) => _reviews.createReview(review, isJobCompleted: isJobCompleted);
  Stream<List<ReviewModel>> getFreelancerReviews(String freelancerId) => _reviews.getFreelancerReviews(freelancerId);

  // ==================== JOBS ====================
  Future<String> createJob(JobModel job) => _jobs.createJob(job);
  Future<JobModel?> getJob(String jobId) => _jobs.getJob(jobId);
  Stream<List<JobModel>> getJobs({String? category, String? state, String? locality, String? status, int limit = 50}) => 
      _jobs.getJobs(category: category, state: state, locality: locality, status: status, limit: limit);
  Stream<List<JobModel>> getFreelancerJobs(String freelancerId) => _jobs.getFreelancerJobs(freelancerId);
  Stream<List<JobModel>> getClientJobs(String clientId) => _jobs.getClientJobs(clientId);
  Future<void> updateJob(String jobId, Map<String, dynamic> data) => _jobs.updateJob(jobId, data);
  ...
  Stream<List<ProposalModel>> getFreelancerProposals(String freelancerId) => _jobs.getFreelancerProposals(freelancerId);
  Future<String> createProposal(ProposalModel proposal) => _jobs.createProposal(proposal);
  Future<void> acceptProposal(ProposalModel proposal) => _jobs.acceptProposal(proposal);
  Future<void> updateProposalStatus(String proposalId, String status) => _jobs.updateProposalStatus(proposalId, status);
  
  // ==================== OFFERS ====================
  Future<void> createOffer(OfferModel offer) => _jobs.createOffer(offer);
  Stream<List<OfferModel>> getOffers(String requestId) => _jobs.getOffers(requestId);
  Future<int> getUserOfferCount(String requestId, String userId) => _jobs.getUserOfferCount(requestId, userId);
  ...
  Future<void> markMessagesAsRead(String chatId, String userId) => _chat.markAsRead(chatId, userId);
  Future<void> updateTypingStatus(String chatId, String userId, bool isTyping) => _chat.updateTypingStatus(chatId, userId, isTyping);

  // ==================== NOTIFICATIONS ====================
  Stream<List<NotificationModel>> getNotifications(String userId) => _notifications.getNotifications(userId);
  ...
  // ==================== PAYMENTS ====================
  Future<String> createPayment(PaymentModel payment) => _payments.createPayment(payment);
  Future<PaymentModel?> getPayment(String paymentId) => _payments.getPayment(paymentId);
  Stream<List<PaymentModel>> getJobPayments(String jobId) => _payments.getJobPayments(jobId);
  Stream<List<PaymentModel>> getUserPayments(String userId, {bool isClient = true}) => _payments.getUserPayments(userId, isClient: isClient);
  Future<void> updatePaymentStatus(String paymentId, PaymentStatus status) => _payments.updatePaymentStatus(paymentId, status);

  // ==================== REPORTS ====================
  Future<void> submitReport(ReportModel report) => _reports.submitReport(report);

  // ==================== APP CONFIG & LOGS ====================
  Future<Map<String, dynamic>> getAppVersionInfo() => _app.getAppVersionInfo();
  Future<String> createContactLog(ContactLogModel log) => _app.createContactLog(log);
  Future<ContactLogModel?> getContactLog(String contacterId, String freelancerId) => _app.getContactLog(contacterId, freelancerId);
  Future<bool> hasContactLog(String contacterId, String freelancerId) => _app.hasContactLog(contacterId, freelancerId);
  Future<void> markContactAsReviewed(String logId) => _app.markContactAsReviewed(logId);
  
  // Legacy / Batch / Special
  Future<void> updateUserProfileImages(String userId, String? imageUrl, String? userName) => _users.updateUserProfileImages(userId, imageUrl, userName);

  // Requests
  Future<void> createRequest(RequestModel request) => _requests.createRequest(request);
  Stream<List<RequestModel>> getRequests() => _requests.getGlobalRequests();
  Future<RequestModel?> getRequestById(String requestId) => _requests.getRequest(requestId);
  Future<void> deleteRequest(String requestId) => _requests.deleteRequest(requestId);
  
  // Cleanup
  Future<void> deleteAllUserData(String userId) => _users.deleteAllUserData(userId);
}

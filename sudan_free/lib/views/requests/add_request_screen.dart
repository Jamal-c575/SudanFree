...
class _AddRequestBottomSheetState extends State<AddRequestBottomSheet> {
  ...
  String _locale(BuildContext context) => context.read<LocaleProvider>().locale.languageCode;
  ...
  Future<void> _pickImages() async {
    ...
    if (source == ImageSource.camera) {
      ...
      if (images.isNotEmpty && mounted) {
        setState(() {
          final toAdd = images.take(remaining).map((img) => File(img.path)).toList();
  ...
  Future<void> _startRecording() async {
    try {
      if (await _audioRecorder.hasPermission()) {
        ...
        final path = '${tempDir.path}/request_audio_${DateTime.now().millisecondsSinceEpoch}.m4a';
  ...
  Future<void> _submitRequest() async {
    ...
    if (_textController.text.trim().isEmpty) {
      messenger.showSnackBar(
        SnackBar(content: Text(locale == 'ar' ? 'الرجاء كتابة تفاصيل الطلب' : 'Please write request details'), backgroundColor: AppColors.warning),
      );
      return;
    }
    ...
    if (_selectedCategory == null) {
      messenger.showSnackBar(
        SnackBar(content: Text(locale == 'ar' ? 'الرجاء تحديد نوع العمل المطلوب' : 'Please select the work type'), backgroundColor: AppColors.warning),
      );
      return;
    }
    ...
    try {
      ...
      if (_selectedImages.isNotEmpty) {
        ...
        for (int i = 0; i < _selectedImages.length; i++) {
          ...
          final url = await StorageService().uploadImage(
            _selectedImages[i],
            folder: 'requests/${widget.user.id}/${DateTime.now().millisecondsSinceEpoch}',
          );
      ...
      if (_recordedAudioPath != null) {
        setState(() => _loadingStatus = locale == 'ar' ? 'جاري رفع التسجيل الصوتي...' : 'Uploading audio...');
      ...
      // 2. Create request
      setState(() => _loadingStatus = locale == 'ar' ? 'جاري نشر الطلب...' : 'Publishing request...');
  ...
  List<Map<String, dynamic>> _getCategories(String locale) {
    return [
      {
        'value': locale == 'ar' ? 'صيانة منزلية (سباكة، كهرباء، الخ)' : 'Home Maintenance (Plumbing, Electrical, etc.)',
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            ...
                            style: TextStyle(color: Colors.blue.shade700, height: 1.5, fontSize: 13),
                          ),
                        ),
                      ],
                    ),
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              if (isSelected) ...[
                                ...
                                Icon(Icons.check_circle, size: 16, color: cat['color'] as Color),
                              ],
                            ],
                          ),
                        ),
                  ...
                  // ═══ 3. Image Attachments ═══
                  Text(
                    locale == 'ar' ? 'صور توضيحية (اختياري)' : 'Attached Images (optional)',
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                  ),
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Icon(Icons.add_photo_alternate_outlined, size: 32, color: AppColors.primary.withValues(alpha: 0.7)),
                                  const SizedBox(height: 4),
                                  Text(
                  ...
                  // ═══ 4. Voice Recording ═══
                  Text(
                    locale == 'ar' ? 'تسجيل صوتي (اختياري)' : 'Voice Record (optional)',
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                  ),
                  const SizedBox(height: 4),
                  Text(
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              locale == 'ar' ? 'تم تسجيل الصوت بنجاح (${_audioDuration} ثانية)' : 'Audio recorded successfully (${_audioDuration}s)',
                      ...
                      child: Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 8),
                            Text(
                              locale == 'ar' ? 'اضغط للبدء في تسجيل الصوت' : 'Tap to start recording',
                              style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold),
                            ),
                          ],
                        ),
                  ...
                  Text(
                    locale == 'ar' ? 'موقع العمل (اختياري)' : 'Work Location (optional)',
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                  ),
                  const SizedBox(height: 4),
                  Text(
                  ...
                  CustomDropdown(
                    ...
                    itemLabelBuilder: (item) => SudanLocations.getStateName(item, locale),
                  ),
                  if (_selectedState != null) ...[
                    const SizedBox(height: 16),
                    CustomDropdown(
                      label: locale == 'ar' ? 'المحلية / المدينة' : 'Locality / City',
                      value: _selectedLocality,
                      items: SudanLocations.statesWithLocalities[_selectedState!] ?? [],
                      onChanged: (val) => setState(() => _selectedLocality = val),
                      hint: locale == 'ar' ? 'اختر المحلية' : 'Select locality',
                      prefixIcon: Icons.location_city_outlined,
                      itemLabelBuilder: (item) => SudanLocations.getLocalityName(item, locale),
                    ),
                  ],
                      ...
                      child: _isLoading 
                          ? Row(
                              ...
                              children: [
                                ...
                                const SizedBox(width: 12),
                                Text(_loadingStatus, style: const TextStyle(fontSize: 14)),
                              ],
                            )
                          : Row(
                              mainAxisAlignment: MainAxisAlignment.center,

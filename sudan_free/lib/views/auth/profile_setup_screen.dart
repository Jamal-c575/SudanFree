...
class _ProfileSetupScreenState extends State<ProfileSetupScreen> {
  ...
  bool _isOutsideSudan = true; // fail-closed: assume outside until proven otherwise
      ...
      if (user.role == UserRole.client) {
        for (final s in user.shopInterests) {
          try {
            ...
            if (!_selectedShopInterests.contains(cat)) _selectedShopInterests.add(cat);
          } catch (_) {}
        }
        for (final s in user.serviceInterests) {
          try {
            final cat = JobCategory.values.firstWhere((e) => e.name == s);
            if (!_selectedServiceInterests.contains(cat)) _selectedServiceInterests.add(cat);
          } catch (_) {}
        }
      }
    }
  ...
  Future<void> _checkRegionLocation() async {
    ...
    if (_detectionFailed && widget.existingUser == null) {
      debugPrint('RegionDetection: All APIs failed → restricting to client for safety');
    }
  }

  Future<void> _fetchCurrentLocation() async {
    setState(() { _locationError = null; _isDetectingLocation = true; });
    bool serviceEnabled;
    ...
    try {
      ...
      if (!serviceEnabled) {
        setState(() { _locationError = 'خدمات الموقع مقفلة.'; _isDetectingLocation = false; });
        return;
      }
      ...
      if (permission == LocationPermission.denied) {
        ...
        if (permission == LocationPermission.denied) {
          setState(() { _locationError = 'تم رفض إذن الوصول للموقع.'; _isDetectingLocation = false; });
          return;
        }
      }
      
      if (permission == LocationPermission.deniedForever) {
        setState(() { _locationError = 'تم رفض إذن الموقع بشكل دائم.'; _isDetectingLocation = false; });
        return;
      } 
    } catch (e) {
      setState(() { _locationError = 'خطأ في أذونات الموقع.'; _isDetectingLocation = false; });
      return;
    }
  ...
  Future<void> _verifyLocationWithGPS(String locale) async {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Row(
          children: [
            ...
            Text(locale == 'ar' ? 'جاري التحقق من الموقع عبر GPS...' : 'Verifying location via GPS...'),
          ],
        ),
    ...
    try {
      ...
      if (mounted) setState(() { _isVerifyingGPS = false; });
    }
  }
  ...
  int get _totalSteps => _isServiceProvider ? 5 : 5; // عميل ومقدمو الخدمات لهم 5 خطوات
  bool get _isServiceProvider => _selectedRole == UserRole.freelancer || _selectedRole == UserRole.techService || _selectedRole == UserRole.privateService || _selectedRole == UserRole.shop;
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      // Add freelancer/tech-specific fields
       if (_selectedRole == UserRole.freelancer || _selectedRole == UserRole.techService || _selectedRole == UserRole.privateService) {
        ...
        if (_hourlyRateController.text.isNotEmpty) {
          profileData['hourlyRate'] = double.tryParse(_hourlyRateController.text);
        }
      }
      ...
      if (_selectedRole == UserRole.shop) {
        ...
        profileData['openingHours'] = _openingHoursController.text.trim().isNotEmpty 
            ? _openingHoursController.text.trim() 
            : null;
        profileData['closingHours'] = _closingHoursController.text.trim().isNotEmpty 
            ? _closingHoursController.text.trim() 
            : null;
      }

      // ─── حذف بيانات نوع الحساب القديم عند التحويل ───
      if (widget.existingUser != null && _selectedRole != widget.existingUser!.role) {
        ...
        if ((oldRole == UserRole.freelancer || oldRole == UserRole.techService || oldRole == UserRole.privateService)
      ...
      if (widget.existingUser != null) {
        ...
        success = await authProvider.createUserProfile(
          ...
          locality: _selectedLocality,
          skills: skills,
          hourlyRate: (_selectedRole == UserRole.freelancer || _selectedRole == UserRole.techService || _selectedRole == UserRole.privateService) && _hourlyRateController.text.isNotEmpty
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        controlsBuilder: (context, details) {
          ...
          final isCheckingLocation = _currentStep == 0 && (_isLoadingRegion || _isVerifyingGPS);
  ...
  List<Step> _buildSteps(String locale) {
    final steps = <Step>[
      ...
      Step(
        ...
        content: (_isLoadingRegion && widget.existingUser == null)
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      SizedBox(height: 16),
                      Text('جاري التحقق من موقعك...', style: TextStyle(color: Colors.grey)),
                    ],
                  ),
                ),
              )
          ...
          children: [
            ...
            if ((_isOutsideSudan || _detectionFailed) && widget.existingUser == null)
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: _detectionFailed ? Colors.orange : AppColors.warning,
                    width: 1.5,
                  ),
                ),
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        Icon(
                          _detectionFailed ? Icons.wifi_off_rounded : Icons.public_off,
                          color: _detectionFailed ? Colors.orange : AppColors.warning,
                          size: 22,
                        ),
                        ...
                        Expanded(
                          child: Text(
                            ...
                            style: TextStyle(
                              color: _detectionFailed ? Colors.orange.shade800 : AppColors.warning,
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          _isVerifyingGPS
                              ...
                              : (locale == 'ar' ? 'تحقق من موقعي عبر GPS' : 'Verify my location via GPS'),
                          style: const TextStyle(fontWeight: FontWeight.bold),
                        ),
                        onPressed: _isVerifyingGPS ? null : () => _verifyLocationWithGPS(locale),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppColors.primary,
                          foregroundColor: Colors.white,
                    ...
                    if (_detectionFailed) ...[
                      ...
                      SizedBox(
                        ...
                        child: OutlinedButton.icon(
                          ...
                          label: Text(
                            locale == 'ar' ? 'إعادة المحاولة' : 'Retry Detection',
                            style: const TextStyle(fontWeight: FontWeight.bold),
                          ),
                          onPressed: () {
            ...
            if ((!_isOutsideSudan && !_detectionFailed) || widget.existingUser != null) ...[
              _buildRoleCard(
                ...
                title: locale == 'ar' ? 'مقدم خدمات فنية' : 'Craft Service Provider',
                description: locale == 'ar' ? 'كهربائي، سباك، نجار، وغيرها من الخدمات الميدانية' : 'Electrician, Plumber, Carpenter, etc.',
              ...
              _buildRoleCard(
                ...
                title: locale == 'ar' ? 'مقدم خدمات تقنية' : 'Tech Service Provider',
                description: locale == 'ar' ? 'مبرمج، مصمم، مونتاج، أو العمل عن بعد' : 'Programmer, Designer, Video Editor, or Remote Worker',
              ...
              _buildRoleCard(
                ...
                title: locale == 'ar' ? 'مقدم خدمات خاصة' : 'Private Service Provider',
                description: locale == 'ar' ? 'مدرس خصوصي، محامي، طباخ، مترجم، مرشد سياحي، وغيرها' : 'Private Tutor, Lawyer, Chef, Translator, Tour Guide, etc.',
              ...
              _buildRoleCard(
                ...
                title: locale == 'ar' ? 'صاحب معرض / متجر' : 'Shop / Gallery Owner',
                description: locale == 'ar' ? 'أملك متجرًا أو معرضًا وأعرض منتجاتي' : 'I own a store/gallery and display my products',
            ...
            _buildRoleCard(
              ...
              description: locale == 'ar' ? 'أبحث عن عمال ومستقلين ومنتجات (للتصفح والتفاعل)' : 'I am looking for workers, freelancers, and products (Browse and Interact)',
      ...
      // Step 2: Basic Info
      Step(
        title: Text(locale == 'ar' ? 'المعلومات الأساسية' : 'Basic Information'),
          ...
          child: Column(
            children: [
              ...
              CustomTextField(
                ...
                hint: _selectedRole == UserRole.shop
                    ? (locale == 'ar' ? 'اكتب وصفًا للمتجر ومنتجاته...' : 'Write a description for your shop...')
                    : (locale == 'ar' ? 'اكتب نبذة مختصرة عنك...' : 'Write a short bio about yourself...'),
      ...
      Step(
        ...
        content: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (_selectedRole == UserRole.techService || _selectedRole == UserRole.client)
               Container(
            ...
            Center(
              child: ElevatedButton.icon(
               ...
               label: Text(_isDetectingLocation 
                   ...
                   : (locale == 'ar' ? 'تحديد موقعي التلقائي' : 'Auto Detect My Location')),
               style: ElevatedButton.styleFrom(
            ...
            if (_locationError != null)
               Padding(
                  ...
                  child: Center(child: Text(_locationError!, style: const TextStyle(color: AppColors.error))),
               ),
            ...
            DropdownButtonFormField<String>(
              ...
              items: SudanLocations.states
                  .map((s) => DropdownMenuItem(value: s, child: Text(SudanLocations.getStateName(s, locale))))
                  .toList(),
              onChanged: (v) {
                setState(() {
                  _selectedState = v;
            ...
            if (_selectedState != null) ...[
              ...
              DropdownButtonFormField<String>(
                ...
                items: SudanLocations.getLocalities(_selectedState!)
                    .map((l) => DropdownMenuItem(value: l, child: Text(SudanLocations.getLocalityName(l, locale))))
                    .toList(),
    ...
    if (_selectedRole == UserRole.freelancer || _selectedRole == UserRole.techService || _selectedRole == UserRole.privateService) {
            ...
            children: [
              ...
              Wrap(
                ...
                children: JobCategory.values.where((category) {
                  ...
                  if (_selectedRole == UserRole.techService) {
                    return [
                      JobCategory.webDevelopment, JobCategory.mobileDevelopment,
                      JobCategory.design, JobCategory.writing, JobCategory.translation,
                      JobCategory.marketing, JobCategory.dataEntry, JobCategory.videoEditing,
                      JobCategory.photography, JobCategory.tutoring, JobCategory.teaching
                    ].contains(category);
                  } else if (_selectedRole == UserRole.privateService) {
                    return [
                      JobCategory.privateTutoring, JobCategory.teachingConsultant,
                      JobCategory.eventCatering, JobCategory.baker, JobCategory.pastryChef,
                      JobCategory.waiter, JobCategory.clinicReception, JobCategory.appointmentBooking,
                      JobCategory.clinicInquiry, JobCategory.lawyer, JobCategory.chef,
                      JobCategory.translator, JobCategory.tourGuide, JobCategory.cooking,
                    ].contains(category);
                  } else {
                    return ![
                      JobCategory.webDevelopment, JobCategory.mobileDevelopment,
                  ...
                  final job = JobModel(
                    ...
                    category: category, budgetMin: 0, budgetMax: 0, deadline: DateTime.now(),
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  label: Text(
                    locale == 'ar' ? 'أضف مسمى وظيفي مخصص' : 'Add custom job title',
                    style: const TextStyle(fontWeight: FontWeight.bold),
                  ),
                  style: OutlinedButton.styleFrom(
                    foregroundColor: AppColors.primary,
        ...
        Step(
          ...
          content: Column(
            ...
            children: [
              const SizedBox(height: 16),
              Text(
                locale == 'ar' ? 'اختر تصنيف متجرك' : 'Choose your shop category',
                style: Theme.of(context).textTheme.bodyMedium,
              ),
              const SizedBox(height: 16),
              Wrap(
          ...
          content: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.location_on_outlined, color: Colors.teal, size: 22),
                    const SizedBox(width: 10),
                    Expanded(
                      child: Text(
                        locale == 'ar'
                        ...
                        style: TextStyle(color: Colors.teal.shade700, fontSize: 13, height: 1.4),
                      ),
                    ),
                  ],
                ),
              ...
              // Shop interests
              Text(
                locale == 'ar' ? '🏪 أنواع المتاجر التي تهمك' : '🏪 Shop types you are interested in',
                style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15),
              ),
              const SizedBox(height: 4),
              Text(
                locale == 'ar' ? 'سنعرض لك هذه الأنواع أولاً في قائمة المتاجر' : 'We will show these types first in the shops list',
                ...
                children: ShopCategory.values.map((cat) {
                  ...
                  return FilterChip(
                    ...
                    onSelected: (val) => setState(() {
                      val ? _selectedShopInterests.add(cat) : _selectedShopInterests.remove(cat);
                    }),
                    selectedColor: AppColors.secondary.withValues(alpha: 0.2),
                    checkmarkColor: AppColors.secondary,
                  );
              ...
              // Service interests
              Text(
                locale == 'ar' ? '🔧 الخدمات التي تحتاجها' : '🔧 Services you need',
                style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15),
              ),
              const SizedBox(height: 4),
              Text(
                locale == 'ar' ? 'سنُظهر لك مقدمي هذه الخدمات في الواجهة الرئيسية' : 'We will highlight providers of these services on the main screen',
                style: TextStyle(fontSize: 12, color: Colors.grey[500]),
              ),
              const SizedBox(height: 12),
              Wrap(
                ...
                children: <JobCategory>[
                  JobCategory.electrical, JobCategory.plumbing, JobCategory.carpentry,
                  JobCategory.painting, JobCategory.cleaning, JobCategory.cooking,
                  JobCategory.webDevelopment, JobCategory.mobileDevelopment, JobCategory.design,
                  JobCategory.translation, JobCategory.marketing, JobCategory.videoEditing,
                  JobCategory.photography, JobCategory.privateTutoring, JobCategory.lawyer,
                  JobCategory.carMaintenance,
                ].map((cat) {
                  final isSelected = _selectedServiceInterests.contains(cat);
                  final job = JobModel(
                    id: '', clientId: '', clientName: '', title: '', description: '',
                    category: cat, budgetMin: 0, budgetMax: 0, deadline: DateTime.now(),
                    createdAt: DateTime.now(), updatedAt: DateTime.now(),
                  );
                  return FilterChip(
                    label: Text(job.getCategoryDisplayName(locale)),
                    selected: isSelected,
                    onSelected: (val) => setState(() {
                      val ? _selectedServiceInterests.add(cat) : _selectedServiceInterests.remove(cat);
                    }),
                    selectedColor: AppColors.primary.withValues(alpha: 0.2),
                    checkmarkColor: AppColors.primary,
                  );
      ...
      steps.add(
        Step(
          title: Text(locale == 'ar' ? 'معاً نبني المستقبل' : 'Together we build the future'),
             ...
             child: Column(
               children: [
                 Container(
                    ...
                    child: const Icon(Icons.volunteer_activism_outlined, size: 40, color: AppColors.primary),
                  ),
                 const SizedBox(height: 16),
                 Text(
                   locale == 'ar' ? 'شكراً لدعمك!' : 'Thank you for your support!',
    ...
    if (_isServiceProvider) {
      steps.add(
        Step(
          title: Text(locale == 'ar' ? 'التوثيق (اختياري)' : 'Verification (Optional)'),
          isActive: _currentStep >= 4,
          content: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                      locale == 'ar' ? 'وثّق حسابك لزيادة الثقة' : 'Verify your account to increase trust',
              ...
              if (widget.existingUser != null) ...[
                ...
                Center(
                  child: ElevatedButton.icon(
                    ...
                    label: Text(locale == 'ar' ? 'الانتقال لصفحة التوثيق' : 'Go to Verification Page'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: AppColors.primary,
                      foregroundColor: Colors.white,
      ...
      child: InputDecorator(
        ...
        child: Text(
          controller.text.isNotEmpty 
              ? _formatTimeDisplay(controller.text, Localizations.localeOf(context).languageCode) 
              : (Localizations.localeOf(context).languageCode == 'ar' ? 'اختر الوقت' : 'Select Time'),
          style: TextStyle(
  ...
  Widget _buildRoleCard(
    ...
    final color = role == UserRole.shop ? AppColors.secondary : AppColors.primary;
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const Text('قريباً', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                Text(
                      ...
                      children: [
                        ...
                        if (comingSoon)
                          Container(
                            ...
                            child: const Text('قريباً', style: TextStyle(fontSize: 11, color: Colors.orange, fontWeight: FontWeight.bold)),
                          ),
                      ],
                    ),
                    const SizedBox(height: 4),

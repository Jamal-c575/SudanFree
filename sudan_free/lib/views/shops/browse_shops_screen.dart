...
class _BrowseShopsScreenState extends State<BrowseShopsScreen> 
  ...
  List<UserModel> _filterShops(List<UserModel> shops) {
    final filtered = shops.where((s) {
      ...
      // Category filter
      if (_selectedCategory != null && s.shopCategory != _selectedCategory) return false;
    ...
    if (currentUser != null && currentUser.shopInterests.isNotEmpty && _searchQuery.isEmpty && _selectedCategory == null) {
      filtered.sort((a, b) {
        final aMatch = a.shopCategory != null && currentUser.shopInterests.contains(a.shopCategory!.name);
        final bMatch = b.shopCategory != null && currentUser.shopInterests.contains(b.shopCategory!.name);
                ...
                children: [
                  Expanded(
                    child: SmartSearchField(
                      ...
                      hintText: locale == 'ar' ? 'ابحث عن متجر، معرض، مطعم...' : 'Search for shop, showroom...',
              ...
              child: userProvider.shopError != null
                  ...
                  : (!hasData && userProvider.isLoading)
                      ...
                      : shops.isEmpty && !userProvider.isLoading
                          ...
                          : RefreshIndicator(
                              onRefresh: () async => userProvider.fetchShops(forceRefresh: true),
                              child: GridView.builder(
                                controller: _scrollController,
                                padding: const EdgeInsets.fromLTRB(16, 16, 16, 96),
                                gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                                  crossAxisCount: 2,
                                  childAspectRatio: 0.77, // More height for stability across screens
                                  crossAxisSpacing: 10,
                                  mainAxisSpacing: 10,
                                ),
                                itemCount: shops.length + (userProvider.isLoadingMoreShops ? 1 : 0),
                                itemBuilder: (context, index) {
                                  ...
                                  return ShopCard(
                                    ...
                                    onTap: () => Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (_) => ProfileScreen(userId: shop.id),
                                      ),
                                    ),
                                  );
                                },
  ...
  Widget _buildEmptyState(BuildContext context, String locale) {
    return EmptyStateWidget(
      ...
      title: locale == 'ar' ? 'لا يوجد معارض أو متاجر' : 'No shops found',
      subtitle: locale == 'ar' 
          ? 'لم نتمكن من العثور على أي متجر يطابق معايير البحث. جرب تغيير الفلاتر أو ابحث بكلمات مختلفة.'
          : 'We couldn\'t find any shops matching your criteria. Try changing filters or search terms.',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () => context.read<UserProvider>().fetchShops(forceRefresh: true),
                ...
                children: [
                  SizedBox(
                    ...
                    child: shop.coverImageUrl != null
                        ? CachedNetworkImage(
                            imageUrl: CloudinaryService.getOptimizedUrl(shop.coverImageUrl!, width: 400, quality: 'auto'),
                            fit: BoxFit.cover,
                            memCacheWidth: 400,
                            placeholder: (_, __) => Container(
                              color: AppColors.secondary.withValues(alpha: 0.2),
                              child: const Icon(Icons.store, size: 40, color: AppColors.secondary),
                            ),
                        ...
                        : Container(
                            color: AppColors.secondary.withValues(alpha: 0.2),
                            child: const Center(
                              child: Icon(Icons.store, size: 40, color: AppColors.secondary),
                            ),
                          ),
                  ),
                  // Category Badge
                  ...
                  if (shop.openingHours != null && shop.closingHours != null &&
                      shop.openingHours!.isNotEmpty && shop.closingHours!.isNotEmpty)
                    Positioned(
                      top: 8,
                      left: 8,
                      child: Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                        decoration: BoxDecoration(
                          color: shop.isShopCurrentlyOpen ? AppColors.success : AppColors.error,
                          borderRadius: BorderRadius.circular(8),
                        ),
                        child: Text(
                  ...
                  children: [
                      ...
                      children: [
                        CircleAvatar(
                          ...
                          backgroundImage: shop.profileImageUrl != null
                              ? CachedNetworkImageProvider(CloudinaryService.getOptimizedUrl(shop.profileImageUrl!, width: 100, quality: 'auto'))
                              : null,
                          child: shop.profileImageUrl == null
                              ? const Icon(Icons.store, size: 14, color: AppColors.secondary)
                              : null,
                        ),
                    ...
                    if (shop.bio != null && shop.bio!.isNotEmpty)
                      Padding(
                        ...
                        child: Text(
                          shop.bio!,
                          maxLines: 1, // Reduced to 1 to leave room for location/rating
                          overflow: TextOverflow.ellipsis,
                    ...
                    if (shop.state != null)
                      Row(
                        children: [
                          Icon(Icons.location_on, size: 10, color: Theme.of(context).iconTheme.color?.withValues(alpha: 0.6)),

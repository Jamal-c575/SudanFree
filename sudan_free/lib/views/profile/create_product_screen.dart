...
class _CreateProductScreenState extends State<CreateProductScreen> {
  ...
  static const _predefinedColors = ['أبيض','أسود','رمادي','أحمر','أزرق','أخضر','أصفر','بني','وردي','برتقالي','بنفسجي','ذهبي','فضي'];
  ...
  Future<void> _pickImages() async {
    ...
    final images = await picker.pickMultiImage(imageQuality: 80, maxWidth: 1200);
    if (images.isNotEmpty && mounted) {
      setState(() => _selectedImages.addAll(images.take(remaining).map((e) => File(e.path))));
    }
  }
  ...
  Future<void> _handleSubmit() async {
    ...
    final caption = '${_nameController.text.trim()}\n\n${_descController.text.trim()}'.trim();
    ...
    try {
      ...
      if (mounted) {
        setState(() => _isPosting = false);
        AppErrorHandler.show(context, e, stack, logContext: 'CreateProductScreen');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isAr ? (isEditing ? 'تعديل المنتج' : 'إضافة منتج') : (isEditing ? 'Edit Product' : 'Add Product')),
        actions: [
          TextButton(
            ...
            child: _isPosting
                ...
                : Text(
                    isAr ? (isEditing ? 'حفظ' : 'نشر') : (isEditing ? 'Save' : 'Publish'),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // ── Images ──────────────────────────────────────────────
            _sectionTitle(isAr ? 'صور المنتج *' : 'Product Images *', Icons.photo_library_outlined),
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: AppColors.secondary.withValues(alpha: 0.4), style: BorderStyle.solid),
                        ),
                        child: const Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.add_photo_alternate_outlined, color: AppColors.secondary, size: 32),
                            SizedBox(height: 4),
                            Text('أضف صورة', style: TextStyle(fontSize: 11, color: AppColors.secondary)),
                          ],
                        ),
                      ),
                    ),
                  ...
                  if (isEditing)
                    ...widget.product!.allImageUrls.map((url) => Container(
                      ...
                      child: CachedNetworkImage(
                        imageUrl: CloudinaryService.getOptimizedUrl(url, width: 200),
                        fit: BoxFit.cover,
                      ),
                    )),
                  // New images
            ...
            // ── Product Name ─────────────────────────────────────────
            _sectionTitle(isAr ? 'اسم المنتج *' : 'Product Name *', Icons.label_outline),
            const SizedBox(height: 8),
            TextField(
              controller: _nameController,
              decoration: _inputDeco(isAr ? 'مثال: حذاء رياضي نايك' : 'e.g. Nike Sports Shoe', Icons.label_outline),
            ),

            const SizedBox(height: 16),

            // ── Description ──────────────────────────────────────────
            _sectionTitle(isAr ? 'الوصف' : 'Description', Icons.description_outlined),
            const SizedBox(height: 8),
            TextField(
              controller: _descController,
              maxLines: 3,
              decoration: _inputDeco(isAr ? 'اكتب وصفاً تفصيلياً للمنتج...' : 'Write a detailed description...', Icons.description_outlined),
            ),
            ...
            Row(
              children: [
                Expanded(child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _sectionTitle(isAr ? 'السعر (SDG)' : 'Price (SDG)', Icons.sell_outlined),
                    const SizedBox(height: 8),
                    TextField(
                      controller: _priceController,
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      decoration: _inputDeco(isAr ? '0.00' : '0.00', Icons.sell_outlined),
                    ),
                  ],
                )),
                ...
                Expanded(child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _sectionTitle(isAr ? 'الكمية المتاحة' : 'Quantity', Icons.inventory_2_outlined),
                    const SizedBox(height: 8),
                    TextField(
                      controller: _quantityController,
                      keyboardType: TextInputType.number,
                      decoration: _inputDeco(isAr ? 'مثال: 10' : 'e.g. 10', Icons.inventory_2_outlined),
                    ),
                  ],
                )),
              ],
            ...
            // ── Condition ────────────────────────────────────────────
            _sectionTitle(isAr ? 'حالة المنتج' : 'Product Condition', Icons.star_outline),
            const SizedBox(height: 8),
            Row(
              children: [
                _conditionChip(isAr ? 'جديد' : 'New', 'new', Icons.fiber_new_rounded, Colors.green),
                const SizedBox(width: 12),
                _conditionChip(isAr ? 'مستعمل' : 'Used', 'used', Icons.recycling_rounded, Colors.orange),
              ],
            ),
            ...
            // ── Age Group ────────────────────────────────────────────
            _sectionTitle(isAr ? 'الفئة العمرية المستهدفة' : 'Target Age Group', Icons.people_outline),
            const SizedBox(height: 8),
            Wrap(
            ...
            // ── Sizes ────────────────────────────────────────────────
            _sectionTitle(isAr ? 'المقاسات المتوفرة' : 'Available Sizes', Icons.straighten_outlined),
            const SizedBox(height: 4),
            Text(isAr ? 'اختر من القائمة أو أضف مقاساً مخصصاً' : 'Select from list or add custom size',
            ...
            Row(children: [
              Expanded(child: TextField(
                ...
                decoration: _inputDeco(isAr ? 'مقاس مخصص...' : 'Custom size...', Icons.add),
              )),
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  if (s.isNotEmpty && !_selectedSizes.contains(s)) {
                    setState(() { _selectedSizes.add(s); _customSizeController.clear(); });
                  }
                },
            ...
            if (_selectedSizes.any((s) => !_predefinedSizes.contains(s))) ...[
              const SizedBox(height: 8),
              Wrap(spacing: 8, children: _selectedSizes.where((s) => !_predefinedSizes.contains(s)).map((s) =>
                Chip(label: Text(s), deleteIcon: const Icon(Icons.close, size: 16),
                  onDeleted: () => setState(() => _selectedSizes.remove(s)),
                  backgroundColor: AppColors.secondary.withValues(alpha: 0.1))).toList()),
            ],

            const SizedBox(height: 20),

            // ── Colors ───────────────────────────────────────────────
            _sectionTitle(isAr ? 'الألوان / التنوعات المتوفرة' : 'Available Colors / Variants', Icons.palette_outlined),
            const SizedBox(height: 8),
            Wrap(
              spacing: 8, runSpacing: 8,
            ...
            Row(children: [
              Expanded(child: TextField(
                ...
                decoration: _inputDeco(isAr ? 'لون أو نوع مخصص...' : 'Custom color/variant...', Icons.add),
              )),
              const SizedBox(width: 8),
              ElevatedButton(
                onPressed: () {
                  ...
                  if (c.isNotEmpty && !_selectedColors.contains(c)) {
                    setState(() { _selectedColors.add(c); _customColorController.clear(); });
                  }
                },
            ...
            Container(
              decoration: BoxDecoration(
                color: _hasShipping ? Colors.teal.withValues(alpha: 0.08) : theme.cardColor,
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: _hasShipping ? Colors.teal.withValues(alpha: 0.4) : AppColors.border.withValues(alpha: 0.3)),
              ),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  isAr ? (isEditing ? 'حفظ التغييرات' : 'نشر المنتج') : (isEditing ? 'Save Changes' : 'Publish Product'),
  ...
  Widget _conditionChip(String label, String value, IconData icon, Color color) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: selected ? color : AppColors.border.withValues(alpha: 0.3), width: selected ? 1.5 : 1),
        ),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, size: 18, color: selected ? color : Colors.grey),
          const SizedBox(width: 6),
          Text(label, style: TextStyle(fontWeight: selected ? FontWeight.bold : FontWeight.normal, color: selected ? color : null)),
        ]),
      ),
    );
  }
  ...
  Widget _ageChip(String value, String label) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? AppColors.secondary.withValues(alpha: 0.15) : Theme.of(context).cardColor,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: selected ? AppColors.secondary : AppColors.border.withValues(alpha: 0.3), width: selected ? 1.5 : 1),
        ),
        child: Text(label, style: TextStyle(fontWeight: selected ? FontWeight.bold : FontWeight.normal, color: selected ? AppColors.secondary : null, fontSize: 13)),
      ),
    );
  }
}

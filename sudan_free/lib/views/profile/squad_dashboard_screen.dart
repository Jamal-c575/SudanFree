...
class _SquadDashboardScreenState extends State<SquadDashboardScreen> {
  ...
  Future<void> _refreshSquad() async {
    try {
      final doc = await FirebaseFirestore.instance.collection('squads').doc(_squad.id).get();
  ...
  Future<void> _leaveSquad(String currentUserId) async {
    ...
    if (confirm == true && mounted) {
      ...
      try {
        await FirebaseFirestore.instance.collection('squads').doc(_squad.id).update({
  ...
  Future<void> _transferLeadership(String newLeaderId) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(isAr ? 'هل أنت متأكد من رغبتك في تسليم قيادة المجموعة لهذا العضو؟ لن تتمكن من استعادتها إلا إذا قام هو بإرجاعها لك.' : 'Are you sure you want to transfer leadership to this member? You will lose leader privileges.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text(isAr ? 'إلغاء' : 'Cancel')),
          ElevatedButton(
            onPressed: () => Navigator.pop(ctx, true),
            style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary),
            child: Text(isAr ? 'تأكيد التحويل' : 'Confirm Transfer', style: const TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
  ...
  Future<void> _disbandSquad() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(isAr ? 'هل أنت متأكد؟ لا يمكن التراجع عن هذا الإجراء وسيتم حذف المجموعة نهائياً.' : 'Are you sure? This cannot be undone and the squad will be deleted.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text(isAr ? 'إلغاء' : 'Cancel')),
          ElevatedButton(
            onPressed: () => Navigator.pop(ctx, true),
            style: ElevatedButton.styleFrom(backgroundColor: Colors.red),
            child: Text(isAr ? 'تفكيك نهائي' : 'Disband Forever', style: const TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
    if (confirm == true && mounted) {
      setState(() => _isLoading = true);
      try {
        await FirebaseFirestore.instance.collection('squads').doc(_squad.id).delete();
        if (mounted) {
          Navigator.pop(context); // Exit Dashboard
          Navigator.pop(context); // Exit Profile
        }
        ...
        : SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    CircleAvatar(
                      ...
                      backgroundImage: _squad.squadImageUrl != null ? CachedNetworkImageProvider(_squad.squadImageUrl!) : null,
                      child: _squad.squadImageUrl == null ? const Icon(Icons.group, size: 30, color: AppColors.primary) : null,
                    ),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(_squad.name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                          Text(isLeader ? (isAr ? 'أنت قائد المجموعة' : 'You are the Leader') : (isAr ? 'أنت عضو في المجموعة' : 'You are a Member'),
                              style: TextStyle(color: isLeader ? AppColors.primary : Colors.grey[600], fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ],
                ...
                if (isLeader) ...[
                  Text(isAr ? 'إدارة المجموعة' : 'Squad Management', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                  
                  // Leader Actions
                  _buildActionCard(
                    icon: Icons.edit,
                    title: isAr ? 'تعديل بيانات المجموعة' : 'Edit Squad Details',
                    subtitle: isAr ? 'تغيير الاسم، الوصف، أو الصورة' : 'Change name, description, or image',
                    color: Colors.blue,
                    onTap: () {
                      Navigator.push(context, MaterialPageRoute(builder: (_) => CreateSquadScreen(squadToEdit: _squad))).then((_) => _refreshSquad());
                    },
                  ),
                  ...
                  _buildActionCard(
                    ...
                    subtitle: isAr ? 'رفع أعمال المجموعة السابقة' : 'Upload previous squad works',
                    color: Colors.green,
                    onTap: () {
                      Navigator.push(context, MaterialPageRoute(builder: (_) => CreatePortfolioProjectScreen(
                        squadId: _squad.id,
                  ...
                  Text(isAr ? 'إدارة الأعضاء (${_squad.memberIds.length})' : 'Manage Members (${_squad.memberIds.length})', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                      ...
                      builder: (context, snapshot) {
                        ...
                        return ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final isThisUserLeader = member.id == _squad.leaderId;
                            
                            return ListTile(
                              leading: CircleAvatar(
                                backgroundImage: member.profileImageUrl != null ? CachedNetworkImageProvider(member.profileImageUrl!) : null,
                                child: member.profileImageUrl == null ? const Icon(Icons.person) : null,
                              ),
                              title: Text(member.name, style: const TextStyle(fontWeight: FontWeight.bold)),
                              subtitle: Text(isThisUserLeader ? (isAr ? 'القائد' : 'Leader') : (isAr ? 'عضو' : 'Member'), style: TextStyle(color: isThisUserLeader ? AppColors.primary : null)),
                              trailing: !isThisUserLeader ? PopupMenuButton<String>(
                                onSelected: (value) {
                                  if (value == 'kick') _kickMember(member.id);
                                  else if (value == 'transfer') _transferLeadership(member.id);
                                },
                                itemBuilder: (context) => [
                                  PopupMenuItem(value: 'transfer', child: Text(isAr ? 'تحويل القيادة' : 'Make Leader')),
                                  PopupMenuItem(value: 'kick', child: Text(isAr ? 'طرد العضو' : 'Kick Member', style: const TextStyle(color: Colors.red))),
                                ],
                              ) : null,
                            );
                          },
                  ...
                  _buildActionCard(
                    ...
                    subtitle: isAr ? 'حذف المجموعة بشكل نهائي' : 'Delete the squad permanently',
                    color: Colors.red,
                    onTap: _disbandSquad,
                  ),
                ],
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 16),
                        Text(
                          isAr ? 'هذه لوحة تحكم المجموعة. فقط قائد المجموعة يمكنه تعديل البيانات أو إدارة الأعضاء وإضافة الأعمال.' 
                ...
                if (!isLeader && isMember)
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(isAr ? 'مغادرة المجموعة' : 'Leave Squad', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    ),
                  ),

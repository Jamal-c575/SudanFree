...
class _SmartSearchFieldState extends State<SmartSearchField> {
  ...
  void _addCompositeSuggestions(String query, Set<String> results) {
    ...
    final topLocations = ['الخرطوم', 'أم درمان', 'بحري', 'ود مدني', 'بورتسودان', 'كسلا', 'القضارف'];
  ...
  List<String> _getQuickSuggestions() {
    switch (widget.searchContext) {
      case SearchContext.freelancers:
        return const ['كهربائي', 'سباك', 'نجار', 'مبرمج', 'مصمم', 'ميكانيكي', 'مدرس', 'دهان'];
      case SearchContext.shops:
        return const ['إلكترونيات', 'ملابس', 'مطعم', 'صيدلية', 'سوبرماركت', 'أثاث', 'جوالات', 'مجوهرات'];
      case SearchContext.community:
  ...
  IconData _getSuggestionIcon(String suggestion) {
    ...
    // حرف وخدمات
    if (lower.contains('سباك') || lower.contains('سباكه')) return Icons.plumbing;
    if (lower.contains('كهرب')) return Icons.electrical_services;
    if (lower.contains('نجار')) return Icons.carpenter;
    if (lower.contains('دهان') || lower.contains('نقاش')) return Icons.format_paint;
    if (lower.contains('ميكانيك')) return Icons.build;
    if (lower.contains('مصمم') || lower.contains('تصميم')) return Icons.design_services;
    if (lower.contains('مبرمج') || lower.contains('مطور')) return Icons.code;
    if (lower.contains('مدرس') || lower.contains('معلم') || lower.contains('تدريس')) return Icons.school;
    if (lower.contains('سائق') || lower.contains('نقل') || lower.contains('توصيل')) return Icons.local_shipping;
    if (lower.contains('طباخ') || lower.contains('شيف')) return Icons.restaurant;
    if (lower.contains('تنظيف') || lower.contains('نظافه')) return Icons.cleaning_services;
    ...
    if (lower.contains('صيانه') || lower.contains('صيانة')) return Icons.build_circle;
    // متاجر
    if (lower.contains('إلكتروني') || lower.contains('الكتروني')) return Icons.devices;
    ...
    if (lower.contains('جوال') || lower.contains('موبايل')) return Icons.phone_android;
    if (lower.contains('مجوهرات')) return Icons.diamond;
    if (lower.contains('متجر') || lower.contains('معرض') || lower.contains('محل')) return Icons.storefront;
    // مواقع
    if (lower.contains('في ') || lower.contains('الخرطوم') || lower.contains('أم درمان') ||
        lower.contains('بحري') || lower.contains('ود مدني') || lower.contains('بورتسودان')) {

...
class InternalLinkPreviewWidget extends StatefulWidget {
  ...
  @override
  State<InternalLinkPreviewWidget> createState() => _InternalLinkPreviewWidgetState();

  /// Helper to check if a URL is an internal deep link
  static bool isInternalLink(String url) {
    return url.contains('sudanfree.com/sudan-free.html') || url.contains('jamall123.github.io/HOME_WEB');
  }
}
...
class _InternalLinkPreviewWidgetState extends State<InternalLinkPreviewWidget> {
        ...
        child: Row(
          children: [
            CircleAvatar(
              ...
              child: user.profileImageUrl == null ? Icon(Icons.person, color: AppColors.primary) : null,
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    user.jobTitle ?? (user.role == UserRole.shop ? 'متجر' : 'مستقل'),
  ...
  Widget _buildPostCard(PostModel post, bool isProduct) {
    ...
    return GestureDetector(
      onTap: () {
        if (isProduct) {
          Navigator.push(context, MaterialPageRoute(builder: (_) => ProductDetailScreen(product: post)));
        } else {
                  ...
                  Row(
                    children: [
                      CircleAvatar(
                        ...
                        child: post.userImageUrl == null ? const Icon(Icons.person, size: 12) : null,
                      ),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          post.userName,
                          style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: isDark ? Colors.white70 : Colors.black54),
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                        ),
                      ),

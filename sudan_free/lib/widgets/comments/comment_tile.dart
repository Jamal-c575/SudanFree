...
class CommentTile extends StatelessWidget {
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  _buildStyledContent(context, comment.content, isReply, comment.mentionedNames),
  ...
  Widget _buildStyledContent(BuildContext context, String content, bool isReply, List<String> mentionedNames) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    ...
    // 2. Prepare URL Regex
    const String urlPattern = r'(?:https?://|www\.)[^\s<>\[\]{}|\\^`\u0600-\u06FF]+';

    // 3. Combined Regex with Groups
    final combinedRegex = RegExp('($mentionPattern)|($urlPattern)', caseSensitive: false);
  ...
  String _timeAgo(DateTime dateTime, BuildContext context) {
    ...
    final locale = AppLocalizations.of(context)!.localeName;
    if (diff.inDays > 0) return locale == 'ar' ? 'منذ ${diff.inDays} يوم' : '${diff.inDays}d ago';
    if (diff.inHours > 0) return locale == 'ar' ? 'منذ ${diff.inHours} ساعة' : '${diff.inHours}h ago';
    if (diff.inMinutes > 0) return locale == 'ar' ? 'منذ ${diff.inMinutes} دقيقة' : '${diff.inMinutes}m ago';
